...
class Observation(ABC):
    ...
    def traci_based_process(self, subscription=None, simulator=None, vehicle=None):
        ...
        obs = {
            "Ego": Observation.pre_process_subscription(
                subscription, simulator, veh_id=self.ego_id, vehicle=vehicle
            )
        }
        ...
        obs["RightLead"] = simulator.get_neighboring_leading_vehicle(
            self.ego_id, "right"
        )
        obs["Foll"] = simulator.get_following_vehicle(self.ego_id)
        obs["LeftFoll"] = simulator.get_neighboring_following_vehicle(
            self.ego_id, "left"
        )
        obs["RightFoll"] = simulator.get_neighboring_following_vehicle(
            self.ego_id, "right"
        )
        return obs
    ...
    @staticmethod
    # @profile
    def pre_process_subscription(
        subscription, simulator, veh_id=None, vehicle=None, distance=0.0
    ):
        ...
        veh["could_drive_adjacent_lane_left"] = simulator.get_vehicle_lane_adjacent(
            veh_id, 1
        )
        veh["could_drive_adjacent_lane_right"] = simulator.get_vehicle_lane_adjacent(
            veh_id, -1
        )

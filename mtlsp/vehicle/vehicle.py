...
from decisionmodels.D2RL.mtlsp.observation.observation import Observation
...
class Vehicle(object):
    ...
    def act(self, action):
        ...
        if action["lateral"] == "central":
            ...
            self.simulator.change_vehicle_sublane_dist(
                self.id, -current_lane_offset, self.step_size
            )
            self.simulator.change_vehicle_speed(
                self.id, controlled_acc, self.action_step_size
            )
        else:
            self.simulator.change_vehicle_lane(
                self.id, action["lateral"], self.lc_duration
            )
            self.simulator.change_vehicle_speed(
                self.id, controlled_acc, self.lc_duration
            )

    def is_action_legal(self, action):
        if action["lateral"] == "left" and not self.simulator.get_vehicle_lane_adjacent(
            self.id, 1
        ):
            return False
        if action[
            "lateral"
        ] == "right" and not self.simulator.get_vehicle_lane_adjacent(self.id, -1):
            return False
        return True
    ...
    def update(self):
        ...
        """
        if (
            self.controller.action is not None
            and not self.controlled_flag
            and self.is_action_legal(self.controller.action)
        ):
            ...
            if (
                self.controller.action["lateral"] == "left"
                or self.controller.action["lateral"] == "right"
            ):
    ...
    def observation(self):
        ...
        """
        if (
            not self._recent_observation
            or self._recent_observation.time_stamp != self.simulator.get_time()
    ...
    def _get_observation(self):
        ...
        """
        obs = self.observation_method(
            ego_id=self.id, time_stamp=self.simulator.get_time()
        )
        obs.update(
            self.simulator.get_vehicle_context_subscription_results(self.id),
            self.simulator,
            self,
        )
        return obs

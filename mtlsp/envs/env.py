from decisionmodels.D2RL.mtlsp.vehicle.vehicle import VehicleList, Vehicle
...
class BaseEnv(object):
    def __init__(
        self, global_controller_dict, independent_controller_dict, info_extractor
    ):
        ...
        for veh_type in self.global_controller_dict:
            self.global_controller_instance_list.append(
                self.global_controller_dict[veh_type](self, veh_type)
            )
    ...
    def initialize(self):
        self.episode_info = {
            "id": self.simulator.episode,
            "start_time": self.simulator.get_time(),
            "end_time": None,
        }

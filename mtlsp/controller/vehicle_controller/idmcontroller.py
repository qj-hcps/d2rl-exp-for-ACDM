...
class IDMController(BaseController):
    ...
    def install(self):
        ...
        self.vehicle.simulator.set_vehicle_color(
            self.vehicle.id, self.vehicle.color_red
        )
    ...
    def decision(observation):
        ...
        # Longitudinal: IDM
            ...
            tmp_acc = acceleration(
                ego_vehicle=ego_vehicle, front_vehicle=front_vehicle, mode=veh_type
            )
    ...
    def mobil_gain(lane_index, observation):
        ...
        new_following_a = acceleration(
            ego_vehicle=new_following, front_vehicle=new_preceding, mode=mode
        )
        new_following_pred_a = acceleration(
            ego_vehicle=new_following, front_vehicle=ego, mode=mode
        )

        old_preceding, old_following = observation["Lead"], observation["Foll"]
        self_pred_a = acceleration(
            ego_vehicle=ego, front_vehicle=new_preceding, mode=mode
        )
        ...
        old_following_a = acceleration(
            ego_vehicle=old_following, front_vehicle=ego, mode=mode
        )
        old_following_pred_a = acceleration(
            ego_vehicle=old_following, front_vehicle=old_preceding, mode=mode
        )
        gain = (
            self_pred_a
            - self_a

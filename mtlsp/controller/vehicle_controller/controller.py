...
class Controller(ABC):
    ...
    def __init__(
        ...
        controllertype="DummyController",
    ):
    ...
    def install(self):
        ...
        self.vehicle.simulator.set_vehicle_color(
            self.vehicle.id, self.vehicle.color_yellow
        )
...
class BaseController(Controller):
    ...
    def __init__(
        ...
        subscription_method=Simulator.subscribe_vehicle_ego,
        controllertype="BaseController",
    ):
        super().__init__(
    ...
    def _check_longitudinal_safety(obs, pdf_array, lateral_result=None, CAV_flag=False):
        ...
        for i in range(len(pdf_array) - 1, -1, -1):
            ...
            if f_veh_info is not None:
                ...
                self_v_2, f_v_2 = max(
                    ego_info["velocity"] + acc, BaseController.v_low
                ), max(
                    (f_veh_info["velocity"] + BaseController.acc_low),
                    BaseController.v_low,
                )
                dist_r = (self_v_2**2 - BaseController.v_low**2) / (
                    2 * abs(BaseController.acc_low)
                )
                dist_f = (f_v_2**2 - BaseController.v_low**2) / (
                    2 * abs(BaseController.acc_low)
                ) + BaseController.v_low * (f_v_2 - self_v_2) / BaseController.acc_low
        ...
        for lane_index, nearby_veh in zip(lane_change_dir, nearby_vehs):
            if pdf_array[lane_index] != 0:
                ...
                if f_veh is not None:
                    ...
                    r = f_veh["distance"]
                    dis_change = (
                        rr * BaseController.ACTION_STEP
                        + 0.5 * BaseController.acc_low * (BaseController.ACTION_STEP**2)
                    )
                    r_1 = r + dis_change  # 1s
                    ...
                    if r_1 <= safety_buffer or r <= safety_buffer:
                        ...
                        self_v_2, f_v_2 = max(
                            CAV_info["velocity"], BaseController.v_low
                        ), max(
                            (f_veh["velocity"] + BaseController.acc_low),
                            BaseController.v_low,
                        )
                        dist_r = (self_v_2**2 - BaseController.v_low**2) / (
                            2 * abs(BaseController.acc_low)
                ...
                if r_veh is not None:
                    ...
                    dis_change = (
                        rr * BaseController.ACTION_STEP
                        - 0.5
                        * BaseController.acc_high
                        * (BaseController.ACTION_STEP**2)
                    )
                    r_1 = r + dis_change
                    ...
                    if r_1 <= safety_buffer or r <= safety_buffer:
                        ...
                        self_v_2, r_v_2 = min(
                            CAV_info["velocity"], BaseController.v_high
                        ), min(
                            (r_veh["velocity"] + BaseController.acc_high),
                            BaseController.v_high,
                        )
                        dist_r = (r_v_2**2 - BaseController.v_low**2) / (
                            2 * abs(BaseController.acc_low)
                        )
                        dist_f = (self_v_2**2 - BaseController.v_low**2) / (
                            2 * abs(BaseController.acc_low)
                        ) + BaseController.v_low * (
                            -r_v_2 + self_v_2
                        ) / BaseController.acc_low

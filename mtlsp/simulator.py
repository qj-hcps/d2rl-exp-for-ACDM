...
import numpy as np
import math
from decisionmodels.D2RL.mtlsp.network.trafficnet import TrafficNet
import time
...
class Simulator(object):
        ...
        if self.sumo_control_state:
            ...
            if self.output is not None:
                ...
                if "traj" in self.output:
                    ...
                    traj_output_path = os.path.join(
                        self.experiment_path, traj_output_filename
                    )
                    sumoCmd += ["--amitran-output", traj_output_path]
                if "fcd_all" in self.output:
                    fcd_output_filename = self.output_filename + ".fcd_all.xml"
                    fcd_output_path = os.path.join(
                        self.experiment_path, "crash", fcd_output_filename
                    )
                    sumoCmd += [
                        "--fcd-output",
                        fcd_output_path,
                ...
                if "lc" in self.output:
                    ...
                    lc_output_path = os.path.join(
                        self.experiment_path, lc_output_filename
                    )
                ...
                if "collision" in self.output:
                    ...
                    collision_output_path = os.path.join(
                        self.experiment_path, collision_output_filename
                    )
    ...
    def _add_vehicle_to_sumo(
        ...
        if libsumo_flag:
            traci.vehicle.add(
                v.id, v.routeID, typeID=typeID, departSpeed=str(v.initial_speed)
            )
        else:
            traci.vehicle.add(
                v.id,
    ...
    def delete_vehicle(self, vehID):
        ...
        if (vehID in self.env.av_list) or (vehID in self.env.bv_list):
            ...
            print(
                f"WARNING: You are deleting vehicle id {vehID} that is not in the current environment!"
            )
    ...
    def get_leading_vehicle(self, vehID):
        ...
        leader_info = traci.vehicle.getLeader(
            vehID, dist=self.config["max_obs_range"]
        )  # empty leader: None
    ...
    def get_neighboring_leading_vehicle(self, vehID, dir):
        ...
        # (it does not include the minGap of the ego vehicle)
        if dir == "left":
            leader_info = traci.vehicle.getNeighbors(
                vehID, 2
            )  # empty leftleader: len=0
        elif dir == "right":
            leader_info = traci.vehicle.getNeighbors(
                vehID, 3
            )  # empty rightleader: len=0
        else:

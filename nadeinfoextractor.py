...
class NADEInfoExtractor(InfoExtractor):
    ...
    def get_terminate_info(self, stop, reason, additional_info):
        ...
        if stop:
            ...
            #     # print("CRASH WEIGHT RESULT:", self.weight_result)
                ...
                save_dir = os.path.join(self.save_dir, "tested_and_safe")
                if self.meet_log_criteria(
                    self.episode_log["ttc_step_info"],
                    self.episode_log["distance_step_info"],
                ):
                    with open(
                        save_dir
                        + "/"
    ...
    def get_snapshot_info(self, control_info=None):
        ...
        try:
            (
                self.episode_log["distance_step_info"][time_step],
                self.episode_log["ttc_step_info"][time_step],
            ) = self.get_current_ttc()
        except:
            pass
        ...
        try:
            self.episode_log["criticality_step_info"][
                time_step
            ] = self.get_criticality_this_step()

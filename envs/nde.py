...
import bisect
import numpy as np
from decisionmodels.D2RL.mtlsp.envs.env import BaseEnv
...
from decisionmodels.D2RL.mtlsp.vehicle.vehicle import Vehicle
from decisionmodels.D2RL.mtlsp.logger.infoextractor import InfoExtractor
...
class NDE(BaseEnv):
    def __init__(
        ...
        super().__init__(
            global_controller_dict={
                "BV": BVGlobalController,
                "CAV": AVGlobalController,
            },
    ...
    def get_av_obs(self):
        ...
        try:
            for veh in return_information:
                try:
                    if (
                        return_information[veh]
                        and return_information[veh]["veh_id"] != "CAV"
                    ):  # has vehicle
    ...
    def get_av_ttc(self):
        ...
        if cav_observation["Lead"] is not None:
            distance_front, ttc_front = self.get_ttc(
                cav_observation["Lead"], cav_observation["Ego"]
            )
        else:
            distance_front, ttc_front = 10000, 10000
        if cav_observation["Foll"] is not None:
            distance_back, ttc_back = self.get_ttc(
                cav_observation["Ego"], cav_observation["Foll"]
            )
        else:
    ...
    def generate_ndd_flow_on_single_lane(self, lane_id, gen_length=conf.gen_length):
        ...
        if ("CAV" not in self.vehicle_list) or self.vehicle_list[
            "CAV"
        ].initial_lane_id != lane_id:  # if CAV is not on current lane
            ...
            tmp_speed_position = {
                "speed": cav_speed,
                "position": self.vehicle_list["CAV"].initial_position,
            }
            bv_before_av_generation_flag = (
                True  # will be disabled after the bv ahead of av is generated
            )

        tmp_speed, tmp_position = (
            tmp_speed_position["speed"],
            tmp_speed_position["position"],
        )

        while tmp_position <= gen_length:
            tmp_speed_position = self.generate_ndd_vehicle(
                back_speed_position=tmp_speed_position,
                lane_id=lane_id,
                bv_before_av_generation_flag=bv_before_av_generation_flag,
            )
            tmp_speed, tmp_position = (
                tmp_speed_position["speed"],
                tmp_speed_position["position"],
    ...
    def generate_ndd_vehicle(
        self, back_speed_position=None, lane_id=None, bv_before_av_generation_flag=False
    ):
        ...
        if bv_before_av_generation_flag:
            ...
            speed, position = self.safe_av_bv_generation(
                cav_speed_position["speed"],
                cav_speed_position["position"],
                speed,
                position,
            )
    ...
    def generate_FF_vehicle(self, back_speed_position=None, direction=1):
        ...
        if back_speed_position is not None:
            ...
            pos_generate = (
                back_speed_position["position"]
                + (conf.ff_dis + rand_position + conf.LENGTH) * direction
            )
            return {"speed": rand_speed, "position": pos_generate}
        else:
            raise Exception("Warning: generating FF vehicle with no back vehicle")
    ...
    def generate_CF_vehicle(self, back_speed_position=None, direction=1):
        ...
        if back_speed_position["speed"] < conf.v_low:
            ...
            presum_list = conf.presum_list_forward[
                conf.v_to_idx_dic[int(back_speed_position["speed"])]
            ]
        random_number = np.random.uniform()
        r_idx, rr_idx = divmod(
            bisect.bisect_left(presum_list, random_number), conf.num_rr
        )
        try:
    ...
    def safe_av_bv_generation(self, cav_speed, cav_position, bv_speed, bv_position):
        min_distance = 15
        time_headway = 1
        required_bv_position = (
            cav_position + min_distance + max(0, time_headway * (cav_speed - bv_speed))
        )
    ...
    def _terminate_check(self):
        ...
        if bool(collision) and "CAV" in collision:
            ...
            self.simulator.get_vehicle_position("CAV")[0] > 800.0
        ):  # This value is different for different experiments
            reason = {4: "CAV reaches 800 m"}
            stop = True
        if stop:
            self.episode_info["end_time"] = (
                self.simulator.get_time() - self.simulator.step_size
            )

...
from decisionmodels.D2RL.mtlsp import utils
from decisionmodels.D2RL.controller.nadecontroller import NADEBackgroundController
import numpy as np
from decisionmodels.D2RL.controller.nddcontroller import NDDController
import decisionmodels.D2RL.conf.conf as conf
...
class Traj:
    def __init__(
        self, x_lon, x_lat, v_lon, v_lat, lane_index, lane_list_info=conf.lane_list
    ):
        self.traj_info = {}
    ...
    def crop(self, start_time, end_time):
        ...
        for time in self.traj_info:
            if (float(time) < start_time and not isclose(start_time, float(time))) or (
                float(time) > end_time and not isclose(end_time, float(time))
            ):
    ...
    def predict_with_action(self, action, start_time=0.0, time_duration=1.0):
        ...
        predict_traj_info["v_lon"] = (
            ini_traj_info["v_lon"] + acceleration * time_duration
        )
    ...
    def predict_without_action(
        self, start_time=0.0, time_duration=1.0, is_lane_change=None, action=None
    ):
        ini_traj_info = self.traj_info["%.1f" % start_time]
        if time_duration == 1.0:
            predict_traj_info = {}
            acceleration_dict = {"accelerate": 2.0, "still": 0.0, "brake": -4.0}
            if is_lane_change is None:
                if ini_traj_info["v_lat"] > 0.5 and not ini_traj_info["x_lat"] >= max(
                    self.lane_list_info
                ):
                    predict_traj_info["lane_index"] = np.argwhere(
                        self.lane_list_info > ini_traj_info["x_lat"]
                    )[0].item()
                elif ini_traj_info["v_lat"] < -0.5 and not ini_traj_info[
                    "x_lat"
                ] <= min(TreeSearchNADEBackgroundController.lane_list):
                    predict_traj_info["lane_index"] = np.argwhere(
                        self.lane_list_info < ini_traj_info["x_lat"]
                    )[-1].item()
                else:
                ...
                if (
                    is_lane_change
                    and ini_traj_info["v_lat"] > 0.5
                    and not ini_traj_info["x_lat"] >= max(self.lane_list_info)
                ):
                    predict_traj_info["lane_index"] = np.argwhere(
                        self.lane_list_info > ini_traj_info["x_lat"]
                    )[0].item()
            ...
            if isclose(ini_traj_info["v_lat"], 0.0):
                ...
                estimated_lane_change_time = round(
                    abs((new_x_lat - ini_traj_info["x_lat"]) / ini_traj_info["v_lat"]),
                    1,
                )
            if (
                not is_lane_change or estimated_lane_change_time > 1
            ):  # only activate when the vehicle is not lane changing
                predict_traj_info["x_lat"] = self.lane_list_info[
                    int(predict_traj_info["lane_index"])
                ]
                if action is None:
                ...
                self.traj_info["%.1f" % (start_time + time_duration)] = (
                    predict_traj_info
                )
                self.interpolate(
                    start_time=start_time, end_time=start_time + time_duration
                )
            else:
                predict_traj_info["x_lat"] = self.lane_list_info[
                    int(predict_traj_info["lane_index"])
                ...
                predict_traj_info["v_lat"] = 0
                predict_traj_info["v_lon"] = ini_traj_info["v_lon"]
                self.traj_info["%.1f" % (start_time + estimated_lane_change_time)] = (
                    predict_traj_info
                )
                self.predict_without_action(
                    start_time + estimated_lane_change_time,
                    1 - estimated_lane_change_time,
                )
                self.interpolate(
                ...
                self.interpolate(
                    start_time=start_time + estimated_lane_change_time,
    ...
    def interpolate(self, start_time, end_time, time_resolution=0.1):
        ...
        if ("%.1f" % start_time) in self.traj_info and (
            "%.1f" % end_time
        ) in self.traj_info:
            ...
            for time_value in inter_list:
                if isclose(time_value, start_time) or isclose(time_value, end_time):
                    ...
                    tmp_traj = {}
                    tmp_traj["x_lon"] = (
                        start_ratio * ini_info["x_lon"]
                        + (1 - start_ratio) * end_info["x_lon"]
                    )
                    tmp_traj["x_lat"] = (
                        start_ratio * ini_info["x_lat"]
                        + (1 - start_ratio) * end_info["x_lat"]
                    )
                    tmp_traj["v_lon"] = (
                        start_ratio * ini_info["v_lon"]
                        + (1 - start_ratio) * end_info["v_lon"]
                    )
                    tmp_traj["v_lat"] = (
                        start_ratio * ini_info["v_lat"]
                        + (1 - start_ratio) * end_info["v_lat"]
                    )
                    tmp_traj["lane_index"] = (
                        (np.abs(self.lane_list_info - tmp_traj["x_lat"]))
                        .argmin()
                        .item()
                    )
...
class TreeSearchNADEBackgroundController(NADEBackgroundController):
    ...
    def update_single_vehicle_obs_no_action(
        veh, duration=1.0, is_lane_change=None, action=None
    ):
        initial_traj = Traj(
            veh["position"][0],
            veh["position"][1],
    ...
    def is_CF(cav_obs, bv_obs):
        ...
        if bv_r1 is not None and bv_r1["veh_id"] == CAV_id:
            ...
            bv_v, bv_range_CAV, bv_rangerate_CAV = (
                bv_obs["Ego"]["velocity"],
                bv_obs["Ego"]["position"][0]
                - cav_obs["Ego"]["position"][0]
                - conf.LENGTH,
                bv_obs["Ego"]["velocity"] - cav_obs["Ego"]["velocity"],
            )
        if bv_f1 is not None and bv_f1["veh_id"] == CAV_id:
            ...
            bv_v, bv_range_CAV, bv_rangerate_CAV = (
                ...
                - bv_obs["Ego"]["position"][0]
                - conf.LENGTH,
                cav_obs["Ego"]["velocity"] - bv_obs["Ego"]["velocity"],
            )
    ...
    def update_single_vehicle_obs(veh, action, duration=conf.simulation_resolution):
        ...
        if (action == "left" or action == "right") and not utils.is_lane_change(veh):
            ...
            new_veh, traj = (
                TreeSearchNADEBackgroundController.update_single_vehicle_obs_no_action(
                    veh, duration, action
                )
            )
        return new_veh, traj
    ...
    def traj_to_obs(prev_full_obs, full_traj, time):
        ...
        for veh_id in full_traj:
            obs[veh_id]["position"] = (
                full_traj[veh_id].traj_info[time]["x_lon"],
                full_traj[veh_id].traj_info[time]["x_lat"],
            )
    ...
    def update_obs(
        ...
        for veh_id in full_obs:
            ...
            if action:
                ...
                if predicted_full_obs is None or predicted_full_traj is None:
                    new_full_obs[veh_id], trajectory_obs[veh_id] = (
                        TreeSearchNADEBackgroundController.update_single_vehicle_obs(
                            vehicle, action
                        )
                    )
                else:
        ...
        new_full_obs[cav_id], trajectory_obs[cav_id] = (
            TreeSearchNADEBackgroundController.update_single_vehicle_obs(
        ...
        av_pos = new_full_obs["0"]["position"]
    ...
    def _process_info(full_obs, ego_id=None, longi=1, lateral=0):
        ...
        for bv_id in full_obs:
            if bv_id != ego_id:
                ...
                if (
                    bv_lane_index == ego_lane_index + lateral
                    and longi * (bv_lane_pos - ego_lane_pos) >= 0
                ):
    ...
    def leaf_node_check(
        ...
        Returns:
            [type]: [description]
        """
        challenge_array = np.zeros(TreeSearchNADEBackgroundController.ACTION_NUM)
        depth_flag = (
            search_depth == TreeSearchNADEBackgroundController.MAX_TREE_SEARCH_DEPTH
        )
        CF_flag = TreeSearchNADEBackgroundController.is_CF(cav_obs, bv_obs)[0]
        bv_id = bv_obs["Ego"]["veh_id"]
        if CF_flag:
            challenge_array[2:] = (
                TreeSearchNADEBackgroundController.get_CF_challenge_array(
                    cav_obs, bv_obs
                )
            )
    ...
    @staticmethod
    # @profile
    def tree_search_maneuver_challenge(
        full_obs,
        previous_obs,
        traj,
        cav_id,
        ...
        leaf_flag, leaf_challenge_array = (
            TreeSearchNADEBackgroundController.leaf_node_check(
                full_obs,
                previous_obs,
                traj,
                cav_obs,
        ...
        cav_action_dict, bv_action_dict, cav_pdf, bv_pdf = (
            TreeSearchNADEBackgroundController.get_cav_bv_pdf(cav_obs, bv_obs)
        )
        # cav_action_list = ["brake", "still", "accelerate"]
        cav_action_list = ["still"]
        if leaf_flag:
            max_challenge_at_leaf = np.max(leaf_challenge_array)
            new_leaf_challenge_array = max_challenge_at_leaf * np.ones_like(
                leaf_challenge_array
            )
            ...
            for bv_action in bv_action_dict:
                if bv_action_dict[bv_action] == 0:
                    ...
                    for cav_action in cav_action_list:
                        updated_full_obs, trajectory_obs = (
                            TreeSearchNADEBackgroundController.update_obs(
                                full_obs,
                                cav_id,
                                bv_id,
                                bv_action,
                        ...
                        new_challenge_array, updated_bv_pdf = (
                            TreeSearchNADEBackgroundController.tree_search_maneuver_challenge(
                                updated_full_obs,
                                full_obs,
                                trajectory_obs,
                                cav_id,
                        ...
                        discount_factor = 1
                        if search_depth != 0:
                            discount_factor = conf.treesearch_config[
                                "treesearch_discount_factor"
                            ]
                        challenge_tmp = discount_factor * np.sum(
                            new_challenge_array * updated_bv_pdf
                        )
                        challenge_tmp_list.append(challenge_tmp)
                    challenge_array[
                        TreeSearchNADEBackgroundController.ACTION_TYPE[bv_action]
                    ] += max(challenge_tmp_list)
        return challenge_array, bv_pdf

    @staticmethod
    # @profile
    def get_CF_challenge_array(cav_obs, bv_obs):
        CF_info, bv_v, bv_range_CAV, bv_rangerate_CAV = (
            TreeSearchNADEBackgroundController.is_CF(cav_obs, bv_obs)
        )
        if not CF_info:
            raise ValueError("get CF challenge in non-CF mode")
        if CF_info == "CAV_BV":
            return NADEBackgroundController._hard_brake_challenge(
                bv_v, bv_range_CAV, bv_rangerate_CAV
            )
        elif CF_info == "BV_CAV":
            return NADEBackgroundController._BV_accelerate_challenge(
                bv_v, bv_range_CAV, bv_rangerate_CAV
            )
    ...
    def _calculate_criticality(
        ...
        if (
            utils.is_lane_change(bv_obs["Ego"])
            and conf.experiment_config["mode"] != "risk_NDE"
        ):
            # if the vehicle is doing lane change, then do not need to calculate the maneuver challenge
            bv_challenge_array, updated_bv_pdf = np.zeros(
                TreeSearchNADEBackgroundController.ACTION_NUM
            ), np.zeros(TreeSearchNADEBackgroundController.ACTION_NUM)
        else:
            bv_challenge_array, updated_bv_pdf = (
                TreeSearchNADEBackgroundController.tree_search_maneuver_challenge(
                    full_obs,
                    None,
                    None,

...
import bisect
from decisionmodels.D2RL.mtlsp.controller.vehicle_controller.controller import (
    Controller,
    BaseController,
)
...
class NDDController(BaseController):
    ...
    # @property
    def ndd_pdf(self, cavobs, time_step):
        # print(self._recent_ndd_pdf)
        # if self._recent_ndd_pdf["time_step"] != self.vehicle.simulator.get_time():
        if self._recent_ndd_pdf["time_step"] != time_step:
            self._recent_ndd_pdf = self.get_ndd_pdf(
                time_step=time_step,
                obs=self.vehicle.obs,
                cav_obs=cavobs,
            )
        return self._recent_ndd_pdf["pdf"]

    # @profile
    def get_ndd_pdf(self, time_step, obs=None, cav_obs=None, external_use=False):
        _recent_ndd_pdf = {}
        ...
        if not external_use:
            _recent_ndd_pdf["time_step"] = time_step
    ...
    def step(self):
        ...
        if self.vehicle.controlled_duration == 0:
            action_id = np.random.choice(
                len(conf.BV_ACTIONS), 1, replace=False, p=final_pdf
            ).item()
    ...
    def Longitudinal_NDD(obs, vehicle_length=conf.LENGTH):
        ...
        if not f1:  # No vehicle ahead. Then FF
            ...
            if (
                ...
                or not NDDController._check_bound_constraints(
                    rr, conf.rr_low, conf.rr_high
                )
                or not NDDController._check_bound_constraints(
                    v, conf.v_low, conf.v_high
                )
            ):
    ...
    def round_value_lane_change(real_value, value_list, round_item="speed"):
        ...
        if round_item == "speed":
            ...
            value_idx = bisect.bisect_left(value_list, real_value)
            value_idx = (
                value_idx - 1 if real_value != value_list[value_idx] else value_idx
            )
            try:
                assert value_idx <= (len(value_list) - 1)
                assert value_idx >= 0
    ...
    def _LC_prob(surrounding_vehicles, full_obs):
        ...
        if not veh_adj_front and not veh_adj_rear:
            ...
            if (
                (r_adj >= conf.min_r_ignore)
                and (CI_LC_prob is not None)
                and (OL_LC_prob is not None)
            ):
                E_LC_prob = (
                    conf.ignore_adj_veh_prob * OL_LC_prob
            ...
            if (
                (r_adj >= conf.min_r_ignore)
                and (DLC_LC_prob is not None)
                and (SLC_LC_prob is not None)
            ):
                E_LC_prob = (
                    conf.ignore_adj_veh_prob * SLC_LC_prob
    ...
    def _get_One_lead_LC_prob(veh_front, full_obs):
        ...
        # Check bound
        if (
            not NDDController._check_bound_constraints(
                v, conf.one_lead_v_low, conf.one_lead_v_high
            )
            or not NDDController._check_bound_constraints(
                r, conf.one_lead_r_low, conf.one_lead_r_high
            )
            or not NDDController._check_bound_constraints(
                rr, conf.one_lead_rr_low, conf.one_lead_rr_high
            )
        ):
            return 0, None
        ...
        if round_speed <= 23 and conf.OL_LC_low_speed_flag:
            ...
            round_speed, round_speed_idx = NDDController.round_value_lane_change(
                real_value=conf.OL_LC_low_speed_use_v,
                value_list=conf.one_lead_speed_list,
                round_item="speed",
            )

        lane_change_prob = (
            conf.OL_pdf[round_speed_idx, round_r_idx, round_rr_idx, :][0]
            * conf.LANE_CHANGE_SCALING_FACTOR
        )
        LC_related = (v, r, rr, round_speed, round_r, round_rr)

        # chech whether there is LC data in this case
        if (
            conf.OL_pdf[round_speed_idx, round_r_idx, round_rr_idx, :][0] == 0
            and conf.OL_pdf[round_speed_idx, round_r_idx, round_rr_idx, :][1] == 0
        ):
    ...
    def _get_Double_LC_prob(veh_adj_front, veh_adj_rear, full_obs):
        v = full_obs["Ego"]["velocity"]
        v_list, r1_list, r2_list, rr1_list, rr2_list = (
            conf.lc_v_list,
            conf.lc_rf_list,
            conf.lc_re_list,
            conf.lc_rrf_list,
        ...
        if not NDDController._check_bound_constraints(v, conf.lc_v_low, conf.lc_v_high):
            return 0, LC_related
        elif not NDDController._check_bound_constraints(
            r1, conf.lc_rf_low, conf.lc_rf_high
        ):
            return 0, LC_related
        elif not NDDController._check_bound_constraints(
            rr1, conf.lc_rrf_low, conf.lc_rrf_high
        ):
            return 0, LC_related
        elif not NDDController._check_bound_constraints(
            r2, conf.lc_re_low, conf.lc_re_high
        ):
            return 0, LC_related
        elif not NDDController._check_bound_constraints(
            rr2, conf.lc_rre_low, conf.lc_rre_high
        ):
            return 0, LC_related
        ...
        lane_change_prob = (
            conf.DLC_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][0]
            * conf.LANE_CHANGE_SCALING_FACTOR
        )
        ...
        # chech whether there is LC data in this case
        if (
            conf.DLC_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][0] == 0
            and conf.DLC_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][1] == 0
        ):
            lane_change_prob = None
    ...
    def _get_Single_LC_prob(veh_front, veh_adj_front, full_obs):
        ...
        if not NDDController._check_bound_constraints(v, conf.lc_v_low, conf.lc_v_high):
            ...
            r2, conf.lc_rf_low, conf.lc_rf_high
        ):
            return 0, LC_related
        elif not NDDController._check_bound_constraints(
            rr2, conf.lc_rrf_low, conf.lc_rrf_high
        ):
            return 0, LC_related
        ...
        lane_change_prob = (
            conf.SLC_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][0]
            * conf.LANE_CHANGE_SCALING_FACTOR
        )
        ...
        # chech whether there is LC data in this case
        if (
            conf.SLC_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][0] == 0
            and conf.SLC_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][1] == 0
        ):
            lane_change_prob = None
    ...
    def _get_Cut_in_LC_prob(veh_front, veh_adj_rear, full_obs):
        ...
        lane_change_prob = (
            conf.CI_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][0]
            * conf.LANE_CHANGE_SCALING_FACTOR
        )
        ...
        # chech whether there is LC data in this case
        if (
            conf.CI_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][0] == 0
            and conf.CI_pdf[v_idx, r1_idx, rr1_idx, r2_idx, rr2_idx, :][1] == 0
        ):
            lane_change_prob = None
    ...
    def round_to_(val, round_item, round_to_closest):
        ...
        try:
            ...
            round_idx = min(
                range(len(value_list)), key=lambda i: abs(value_list[i] - round_val)
            )
    ...
    def MOBIL_result(obs):
        ...
        if left_LC_flag or right_LC_flag:
            if left_gain >= right_gain:
                left_prob, right_prob = MOBIL_LC_prob, 0.0
            else:
                left_prob, right_prob = 0.0, MOBIL_LC_prob

...
from decisionmodels.D2RL.controller.nddglobalcontroller import NDDBVGlobalController
from decisionmodels.D2RL.mtlsp.vehicle.vehicle import Vehicle, VehicleList
...
class NADEBVGlobalController(NDDBVGlobalController):
    ...
    def step(self):
        ...
        for bv_id in self.controllable_veh_id_list:
            ...
            # print(bv.controller.__class__.__name__, bv.controller.step())
            bv.controller.step()
        ...
        if self.apply_control_permission():
            if conf.experiment_config["mode"] == "NDE":
                ...
                raise ValueError(
                    "conf experiment mode not recognized. should be NDE or D2RL"
                )
    ...
    # @profile
    def select_controlled_bv_and_action_exp(self, CAV_id, vehicle_obs_dict, time_step):
        ...
        """
        num_controlled_critical_bvs = 10
        controlled_bvs_list = []
        controlled_bvs_id_list = []
        vehicle_dict = {}
        bv_pdf_dict = {}
        for key in vehicle_obs_dict.keys():
            vehicle = Vehicle(
                id=key,
                controller=TreeSearchNADEBackgroundController(),
                simulator=self.env.simulator,
            )
            vehicle.obs = vehicle_obs_dict.get(key)
            if key != "0":
                controlled_bvs_id_list.append(key)
                controlled_bvs_list.append(vehicle)
            vehicle_dict[key] = vehicle
        CAV_obs = vehicle_obs_dict.get(CAV_id)
        full_obs = collections.OrderedDict()
        for key in vehicle_obs_dict.keys():
            full_obs[key] = vehicle_obs_dict[key]["Ego"]
        self.env.vehicle_list = VehicleList(vehicle_dict)
        ...
        (
            ...
            ndd_possi_list,
            IS_possi_list,
        ) = self.calculate_criticality_list(
            controlled_bvs_list, CAV_obs, full_obs, time_step
        )
        whole_weight_list = []
        ...
        )  # get D2RL agent observation
        self.control_log["discriminator_input"] = discriminator_input.tolist()
        self.epsilon_value = -1
        underline_drl_action = self.get_underline_drl_action(
            discriminator_input, bv_criticality_list
        )
        ...
        for i in range(len(controlled_bvs_list)):
            ...
            bv_pdf = bv.controller.get_NDD_possi(CAV_obs, time_step)
            bv_pdf_dict[bv.id] = bv_pdf
            combined_bv_criticality_array = bv_criticality_array
            bv_action_idx, weight, ndd_possi, critical_possi, single_weight_list = (
                bv.controller.Decompose_sample_action(
                    np.sum(combined_bv_criticality_array),
                    combined_bv_criticality_array,
                    bv_pdf,
                    underline_drl_action,
                )
            )
            if bv_action_idx is not None:
                bv_action_idx = bv_action_idx.item()
            bv_action_idx_list.append(bv_action_idx), weight_list.append(
                weight
            ), ndd_possi_list.append(ndd_possi), IS_possi_list.append(critical_possi)
        ...
        for i in range(len(controlled_bvs_list)):
            if i in selected_bv_idx:
                if (
                    whole_weight_list[i]
                    and whole_weight_list[i]
                    * self.env.info_extractor.episode_log["weight_episode"]
                    * self.env.initial_weight
                    < conf.weight_threshold
                ):
                    (
                        bv_action_idx_list[i],
                        weight_list[i],
                        ndd_possi_list[i],
                        IS_possi_list[i],
                    ) = (None, None, None, None)
    ...
    def select_controlled_bv_and_action(self):
        ...
        full_obs = self.get_full_obs_from_cav_obs_and_bv_list(
            CAV_obs, controlled_bvs_list
        )
        self.nade_candidates = controlled_bvs_list
        ...
        (
            ...
            ndd_possi_list,
            IS_possi_list,
        ) = self.calculate_criticality_list(controlled_bvs_list, CAV_obs, full_obs)
        whole_weight_list = []
        self.control_log["criticality"] = sum(bv_criticality_list)
    ...
    def apply_control_permission(self):
        for vehicle in self.get_bv_candidates():
            if vehicle.controller.NADE_flag and utils.is_lane_change(
                vehicle.observation.information["Ego"]
            ):
                return False
        return True
    ...
    def pre_load_predicted_obs_and_traj(full_obs):
        ...
        for veh_id in full_obs:
            for action in action_list:
                ...
                predicted_obs[veh_id][action], trajectory_obs[veh_id][action] = (
    ...
    def collect_discriminator_input_simplified(
        self, full_obs, controlled_bvs_list, bv_criticality_list
    ):
        """D2RL agent observation collection"""
        CAV_global_position = list(full_obs["0"]["position"])
        CAV_speed = full_obs["0"]["velocity"]
        ...
        if len(bv_criticality_list):
            ...
            predict_relative_position = (
                vehicle_relative_position[0] + vehicle_relative_speed
            )
            vehicle_info_list.extend(
                vehicle_relative_position
                + [vehicle_relative_speed]
                + [predict_relative_position]
            )
        else:
            vehicle_info_list.extend(
                [-20, -8, -10, -20]
            )  # fill the state space with default values
        if len(vehicle_info_list) < total_bv_info_length:
            vehicle_info_list.extend(
                [-1] * (total_bv_info_length - len(vehicle_info_list))
            )
        ...
        lb_array = np.array(
            ...
            + [CAV_velocity_lb]
            + [weight_lb]
            + [bv_criticality_flag_lb]
            + [bv_criticality_value_lb]
        ...
        ub_array = np.array(
            ...
            + [CAV_velocity_ub]
            + [weight_ub]
            + [bv_criticality_flag_ub]
            + [bv_criticality_value_ub]
    ...
    def calculate_criticality_list(
        self, controlled_bvs_list, CAV_obs, full_obs, time_step
    ):
        (
            bv_criticality_list,
            criticality_array_list,
        ...
        predicted_full_obs, predicted_traj_obs = (
            NADEBVGlobalController.pre_load_predicted_obs_and_traj(full_obs)
        )
        CAV_left_prob, CAV_still_prob, CAV_right_prob = (
            NADEBVGlobalController._get_Surrogate_CAV_action_probability(
                cav_obs=CAV_obs
            )
        )
        for bv in controlled_bvs_list:
            ...
            bv_criticality_list.append(bv_criticality), criticality_array_list.append(
                criticality_array
            )
        return (
            bv_criticality_list,
    ...
    def get_underline_drl_action(self, discriminator_input, bv_criticality_list):
        ...
        if sum(bv_criticality_list) > 0:
            ...
            ):  # using D2RL agent to output the adversarial maneuver probability
                if conf.discriminator_agent is None:
                    conf.discriminator_agent = conf.load_discriminator_agent()
                underline_drl_action = conf.discriminator_agent.compute_action(
                    discriminator_input
                )
                if sum(bv_criticality_list) > 0:
                    print(
    ...
    def _get_Surrogate_CAV_action_probability(cav_obs):
        ...
        max_remaining_LC_prob = (
            1 - conf.epsilon_still_prob - CAV_left_prob - CAV_right_prob
        )
    ...
    def _Mobil_surraget_model(cav_obs, lane_index):
        ...
        self_a = utils.acceleration(ego_vehicle=cav_info, front_vehicle=old_preceding)
    ...
    def _process_cav_context(self, vehicle_id_list):
        ...
        total_vehicle_id_list = list(set(vehicle_id_list) | set(cav_context.keys()))
        for veh_id in total_vehicle_id_list:
            ...
            if distance > conf.cav_obs_range + 5:
                ...
                relative_lane_index = (
                    self.env.simulator.get_vehicles_relative_lane_index("CAV", veh_id)
                )
            cav_surrounding[veh_id] = {
                ...
                "position": bv_pos,
                "prev_action": self.env.vehicle_list[veh_id].observation.information[
                    "Ego"
                ]["prev_action"],

...
class NADEBackgroundController(BASE_NDD_CONTROLLER):
    ...
    # @profile
    def get_NDD_possi(self, cav_obs, time_step):
        return np.array(self.ndd_pdf(cavobs=cav_obs, time_step=time_step))

    # @profile
    def _sample_critical_action(
        self,
        bv_criticality,
        criticality_array,
        ndd_possi_array,
        ...
        critical_possi, ndd_possi = (
            epsilon_pdf_array[bv_action_idx],
            ndd_possi_array[bv_action_idx],
        )
        weight_list = (ndd_possi_array + 1e-30) / (
            epsilon_pdf_array + 1e-30
        )  # avoid zero division
    ...
    def _hard_brake_challenge(v, r, rr):
        ...
        new_r = r + rr
        if new_r <= 2.1:
            CF_challenge_array = np.ones((len(conf.BV_ACTIONS) - 2), dtype=float)
    ...
    def _BV_accelerate_challenge(v, r, rr):
        ...
        index = np.where(
            (conf.BV_CF_state_value == [round_r, round_rr, round_speed]).all(1)
        )
        assert len(index) <= 1
        index = index[0]
        ...
        new_r = r + rr
        if new_r <= 2.1:
            BV_CF_challenge_array = np.ones((len(conf.BV_ACTIONS) - 2), dtype=float)
    ...
    def Decompose_decision(
        ...
        (
            ...
            None,
            None,
            np.zeros(len(conf.ACTIONS), dtype=float),
        )
        bv_id = self.vehicle.id
        bv_pdf = self.get_NDD_possi(CAV, time_step=time_step)
        bv_obs = self.vehicle.obs
        ...
        if not ((0.99999 <= bv_left_prob <= 1) or (0.99999 <= bv_right_prob <= 1)):
            bv_criticality, criticality_array, bv_challenge_array, risk = (
    ...
    def Decompose_sample_action(
        self, bv_criticality, bv_criticality_array, bv_pdf, epsilon=conf.epsilon_value
    ):
        """Sample the critical action of the BV."""
        if epsilon is None:
            epsilon = conf.epsilon_value
        bv_action_idx, weight, ndd_possi, critical_possi, weight_list = (
            None,
            None,
            None,
            None,

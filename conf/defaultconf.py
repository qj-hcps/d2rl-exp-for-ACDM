...
simulation_resolution = (
    1.0  # The time interval(resolution) of the simulation platform (second)
)
# =========== Vehicle parameters =============
LENGTH = 5.0
...
random_env_BV_generation_range_min, random_env_BV_generation_range_max = (
    0,
    300,
)  # ! Not used
initial_CAV_speed = 30  # ! Not used
initial_CAV_position = 400  # ! Not used
ndd_data_path = "./decisionmodels/D2RL/source_data"

CF_joint_pdf = np.load(
    ndd_data_path + "/NDD_DATA/CF/Optimized_CF_joint_pdf_array.npy"
)  # ! Not used
presum_list_forward = np.load(
    ndd_data_path + "/NDD_DATA/Initialization/Optimized_presum_list_forward.npy"
)  # choose prev vehicle according to range and range rate, according to the velocity of the following vehicle
...
random_veh_pos_buffer_start, random_veh_pos_buffer_end = (
    0,
    50,
)  # the first vehicle in the traffic flow will be generated between random_veh_pos_buffer_start and random_veh_pos_buffer_end
...
speed_CDF = list(
    np.load(ndd_data_path + "/NDD_DATA/Initialization/speed_CDF.npy")
)  # NDD distribution to generate the initial speed of ndd vehicles
CF_pdf_array = np.load(
    ndd_data_path + "/NDD_DATA/CF/Optimized_CF_pdf_array.npy"
)  # NDD distribution to generate the longitudinal decision (acceleration) under car following scenarios
FF_pdf_array = np.load(
    ndd_data_path + "/NDD_DATA/FF/Optimized_FF_pdf_array.npy"
)  # NDD distribution to generate longitudinal decisions (acceleration) under free flow scenarios
...
OL_pdf = sparse.load_npz(
    ndd_data_path + "/NDD_DATA/LC/10106_10620_OL_pdf_smoothed_soft_p8_1_tmp.npz"
).todense()
SLC_pdf = sparse.load_npz(
    ndd_data_path + "/NDD_DATA/LC/10119_10630_SLC_pdf_smoothed_soft_p8_1_tmp.npz"
).todense()
DLC_pdf = sparse.load_npz(
    ndd_data_path + "/NDD_DATA/LC/10119_10630_DLC_pdf_smoothed_p8_1_tmp.npz"
).todense()
CI_pdf = sparse.load_npz(
    ndd_data_path + "/NDD_DATA/LC/10119_10630_CI_pdf_smoothed_p8_1_tmp.npz"
).todense()
...
if print_log_flag:
    if v_resolution == 0.2:
        print("Action v resolution is 0.2!")
    if v_resolution == 1:
        print("Action v resolution is 1!")
num_v, num_r, num_rr, num_acc = (
    int(1 + ((v_high - v_low) / v_resolution)),
    int(1 + ((r_high - r_low) / r_resolution)),
    int(1 + ((rr_high - rr_low) / rr_resolution)),
    int(1 + ((acc_high - acc_low) / acc_resolution)),
)
assert ((num_r, num_rr, num_v, num_acc) == CF_pdf_array.shape) and (
    (num_v, num_acc) == FF_pdf_array.shape
)
speed_list, r_list, rr_list, acc_list = (
    list(np.linspace(v_low, v_high, num=num_v)),
...
r_to_idx_dic, rr_to_idx_dic, v_to_idx_dic, v_back_to_idx_dic, acc_to_idx_dic = (
    bidict(),
    bidict(),
    bidict(),
    bidict(),
    bidict(),
)  # Two way dictionary, the front path is: key: real value, value: idx
for i in range(num_r):
    r_to_idx_dic[list(np.linspace(r_low, r_high, num=num_r))[i]] = i
for j in range(num_rr):
    rr_to_idx_dic[list(np.linspace(rr_low, rr_high, num=num_rr))[j]] = j
for k in range(num_v):
    v_to_idx_dic[list(np.linspace(v_low, v_high, num=num_v))[k]] = k
...
one_lead_v_to_idx_dic, one_lead_r_to_idx_dic, one_lead_rr_to_idx_dic = (
    bidict(),
    bidict(),
    bidict(),
)
...
one_lead_speed_list, one_lead_r_list, one_lead_rr_list = (
    list(range(one_lead_v_low, one_lead_v_high + one_lead_v_step, one_lead_v_step)),
    list(range(one_lead_r_low, one_lead_r_high + one_lead_r_step, one_lead_r_step)),
    list(range(one_lead_rr_low, one_lead_rr_high + one_lead_rr_step, one_lead_rr_step)),
)
for i in range(
    int((one_lead_v_high - one_lead_v_low + one_lead_v_step) / one_lead_v_step)
):
    one_lead_v_to_idx_dic[
        list(range(one_lead_v_low, one_lead_v_high + one_lead_v_step, one_lead_v_step))[
            i
        ]
    ] = i
for i in range(
    int((one_lead_r_high - one_lead_r_low + one_lead_r_step) / one_lead_r_step)
):
    one_lead_r_to_idx_dic[
        list(range(one_lead_r_low, one_lead_r_high + one_lead_r_step, one_lead_r_step))[
            i
        ]
    ] = i
for i in range(
    int((one_lead_rr_high - one_lead_rr_low + one_lead_rr_step) / one_lead_rr_step)
):
    one_lead_rr_to_idx_dic[
        list(
            range(
                one_lead_rr_low, one_lead_rr_high + one_lead_rr_step, one_lead_rr_step
            )
        )[i]
    ] = i
...
for i in range(lc_v_num):
    lc_v_to_idx_dic[list(np.linspace(lc_v_low, lc_v_high, num=lc_v_num))[i]] = i
for i in range(lc_rf_num):
    lc_rf_to_idx_dic[list(np.linspace(lc_rf_low, lc_rf_high, num=lc_rf_num))[i]] = i
for i in range(lc_re_num):
    lc_re_to_idx_dic[list(np.linspace(lc_re_low, lc_re_high, num=lc_re_num))[i]] = i
for i in range(lc_rrf_num):
    lc_rrf_to_idx_dic[list(np.linspace(lc_rrf_low, lc_rrf_high, num=lc_rrf_num))[i]] = i
for i in range(lc_rre_num):
    lc_rre_to_idx_dic[list(np.linspace(lc_rre_low, lc_rre_high, num=lc_rre_num))[i]] = i
...
if cav_training:
    ...
    save_folder_target_net = (
        os.path.abspath(".") + "/models/CAV/" + save_folder_name + "/target_net/"
    )
    save_folder_memory = (
        os.path.abspath(".") + "/models/CAV/" + save_folder_name + "/memory/"
    )
    if os.path.exists(main_folder):
        pass
    else:
        os.mkdir(main_folder)
    if os.path.exists(save_folder_dev):
        pass
    else:
        os.mkdir(save_folder_dev)
    if os.path.exists(save_folder_target_net):
        pass
    else:
        os.mkdir(save_folder_target_net)
    if os.path.exists(save_folder_memory):
        pass
    else:
        os.mkdir(save_folder_memory)
# ! Not used
if bv_training:
    bv_main_folder = os.path.abspath(".") + "/models/BV/" + bv_save_folder_name
    bv_save_folder_dev = (
        os.path.abspath(".") + "/models/BV/" + bv_save_folder_name + "/dev/"
    )
    bv_save_folder_target_net = (
        os.path.abspath(".") + "/models/BV/" + bv_save_folder_name + "/target_net/"
    )
    bv_save_folder_memory = (
        os.path.abspath(".") + "/models/BV/" + bv_save_folder_name + "/memory/"
    )
    if os.path.exists(bv_main_folder):
        pass
    else:
        os.mkdir(bv_main_folder)
    if os.path.exists(bv_save_folder_dev):
        pass
    else:
        os.mkdir(bv_save_folder_dev)
    if os.path.exists(bv_save_folder_target_net):
        pass
    else:
        os.mkdir(bv_save_folder_target_net)
    if os.path.exists(bv_save_folder_memory):
        pass
    else:
        os.mkdir(bv_save_folder_memory)

# =========== BV related ================
BV_ACTIONS = {
    0: "LANE_LEFT",
    1: "LANE_RIGHT",
}  # lateral and longitudinal maneuver candidates of BV
num_acc = int(
    ((acc_high - acc_low) / acc_resolution) + 1
)  # number of longitudinal maneuver candidates (31 discrete accelerations) of BV
num_non_acc = len(
    BV_ACTIONS
)  # number of lateral maneuver candidates (left lane change and right lane change) of BV
...
ACTIONS = {
    0: "LANE_LEFT",
    1: "LANE_RIGHT",
}  # lateral and longitudinal maneuver candidates of CAV
num_acc = int(
    ((acc_high - acc_low) / acc_resolution) + 1
)  # number of longitudinal maneuver candidates (31 discrete accelerations) of AV
num_non_acc = len(
    ACTIONS
)  # number of lateral maneuver candidates (left lane change and right lane change) of AV
for i in range(num_acc):
    acc = acc_to_idx_dic.inverse[i]
...
# =========== NDD ENV para ============
safety_guard_enabled_flag, safety_guard_enabled_flag_IDM = (
    False,
    True,
)  # In car following scenarios, if the current state is out of the NDD data bound, or current state has no NDD data, we will use the stochastic IDM model to generate the longitudinal decision, and the safety guard function will be utilized to ensure the safety of the longitudinal decision.
...
longi_safety_buffer, lateral_safety_buffer = (
    2,
    2,
)  # The safety buffer used to longitudinal and lateral safety guard

dis_with_CAV_in_critical_initialization = 300  # ! Not used
critical_ini_start = (
    initial_CAV_position - dis_with_CAV_in_critical_initialization
)  # ! Not used
critical_ini_end = initial_CAV_position  # ! Not used

LANE_CHANGE_SCALING_FACTOR = 1.0  # ! Not used
...
ignore_adj_veh_prob, min_r_ignore = (
    1e-2,
    5,
)  # Probability of the ignoring the vehicle in the adjacent lane
ignore_lane_conflict_prob = 1e-4  # ! Not used
LC_range_threshold = 200  # ! Not used

# ============= Criticality Model Data ================
CF_state_value = scipy.io.loadmat(
    ndd_data_path + "/Decompose/CF/" + "dangerous_state_table.mat"
)["dangerous_state_table"]
CF_challenge_value = scipy.io.loadmat(
    ndd_data_path + "/Decompose/CF/" + "Q_table_little.mat"
)["Q_table_little"]
...
BV_CF_challenge_value = np.load(
    ndd_data_path + "/Decompose/BV_CF/BV_CF_challenge_value.npy"
)
...
epsilon_lane_change_prob = (
    1e-8  # 5% to do lane change even if the gain is zero or even negative
)
SM_MOBIL_max_gain_threshold = (
    1  # m/s^2 when gain is greater than this, the LC probability will be maximum
)
Surrogate_POLITENESS = 0.0
